...
export const DataSourcesModal: React.FC<Props> = ({ isOpen, onClose, onDataParsed, sourceEntries, onMergeAnalytics }) => {
  ...
  const connectToCrawlJob = (jobId: string, type: 'source' | 'target', url: string) => {
    ...
    eventSource.onmessage = (event) => {
      ...
      if (data.type === 'start') {
        setCrawlProgress((prev: any) => ({...prev, [type]: { jobId, status: 'starting', current: 0, total: 0 }}));
      } else if (data.type === 'progress') {
        setCrawlProgress((prev: any) => ({...prev, [type]: { jobId, status: 'crawling', current: data.current, total: data.total, message: data.message }}));
      } else if (data.type === 'done') {
        eventSource.close();
        setCrawlProgress((prev: any) => ({...prev, [type]: { jobId, status: 'done', current: data.results.length, total: data.results.length }}));
        
        setTimeout(() => {
          setIsProcessing(false);
          // In the modal, we trigger onDataParsed which currently updates targetEntries in App
          onDataParsed(data.results);
        }, 500);
